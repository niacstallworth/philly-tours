...
const approveScriptPath = path.join(rootDir, "scripts", "approve-ar-runtime-stop.mjs");
...
function parseNoteToken(noteToken) {
...
function parseQueueCommands(input) {
  ...
  for (const line of lines) {
    ...
    if (!trimmed.startsWith("npm run ar:catalog:approve -- --stop-id ")) {
      continue;
    }

    const match = trimmed.match(/^npm run ar:catalog:approve -- --stop-id (\S+) --note (.+)$/);
    if (!match) {
...
function main() {
  ...
  if (!commands.length) {
    throw new Error("No approval commands found in the provided queue text.");
  }

  for (const command of commands) {
    const approveArgs = [approveScriptPath, "--stop-id", command.stopId, "--note", command.note];
    if (args.dryRun) {
      approveArgs.push("--dry-run");
    }
    if (args.force) {
      approveArgs.push("--force");
    }

    execFileSync(process.execPath, approveArgs, {
      cwd: rootDir,
      stdio: "inherit"
    });
  }
  ...
  console.log(
    `${args.dryRun ? "Previewed" : "Applied"} approval queue for ${commands.length} stop(s).`
  );
}
