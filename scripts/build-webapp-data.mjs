...
const tours =
  Array.isArray(cityPack.tours) && cityPack.tours.length
    ? cityPack.tours
    : evaluateModule(sourcePath, "__tours", /export const tours\s*:\s*Tour\[\]\s*=/);
const narrationCatalogByStopId =
  cityPack.narration?.catalogByStopId ||
  evaluateModule(
...
const narrationScriptMapByStopId =
  cityPack.narration?.scriptMapByStopId ||
  evaluateModule(
...
const derivedArStops = tours.flatMap((tour) =>
  (tour.stops || []).map((stop) => ({
    id: stop.id,
    title: stop.title,
...
const arByStopId =
  cityPack.ar ||
  Object.fromEntries(derivedArStops.map((stop) => [stop.id, stop]));

const arBanner = [
  "// Auto-generated by scripts/build-webapp-data.mjs",
  "// Source: src/data/tours.ts AR fields",
  `window.PHILLY_TOURS_AR = ${JSON.stringify(arByStopId, null, 2)};`,
  ""
].join("\n");
